...
nsHttpServer.prototype =
{
  ...
  _start: function(port, host)
  {
    ...
    try
    {
      ...
      var socket = new ServerSocket(this._port,
                                    false, // true = localhost, false = everybody
                                    maxConnections);
  ...
  //
  // see nsIHttpServer.registerIdentityHandler
  //
  registerIdentityHandler: function(scheme, host, port, handler)
  {
    this._identity.add(scheme, host, port);
    this._handler.registerIdentityHandler(scheme, host, port, handler);
  },
...
ServerHandler.prototype =
{
  ...
  handleResponse: function(connection)
  {
    ...
    var identity = request.scheme + "://" + request.host + ":" +
        request.port;
    ...
    try
    {
      try
      {
        if (identity in this._overrideIdentities)
        {
          dumpn("calling override for identity " + identity);
          this._overrideIdentities[identity](request, response);
        }
  ...
  registerIdentityHandler: function(scheme, host, port, handler)
  {
    this._handlerToField(handler, this._overrideIdentities, scheme +
        "://" + host + ":" + port);
  },
